...
- 논문으로 받은 상은 `awards:` 목록에 `text`(필수), `url`·`source`(선택)를 적으면 논문 아래에 한 줄씩 굵게 표시되고, `url`이 있으면 옆에 작은 링크가 붙습니다.
- `type: preprint`인 항목은 연도와 상관없이 맨 위 "Preprints" 묶음에 모입니다(`year`는 그대로 적어 두세요).
...
- 루트의 PDF와 노트북 파일은 그대로 복사되어 예전 주소 그대로 열립니다.
- 강의 페이지(`MA1407.html` 등)는 맨 위 `---` 사이에 제목·학기·시간·그림을 적고, 그 아래 내용은 예전처럼 간단한 HTML로 고칩니다. 강의 한 줄은 `<li> Tue 9/02: 주제 (<a href="파일.pdf">note</a>)</li>` 형식 그대로 추가하면 날짜·주제·버튼으로 자동 배치됩니다. 주소(`/MA1407`)는 그대로입니다.
